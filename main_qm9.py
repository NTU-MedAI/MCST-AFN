...
def main():
    ...
    args.epochs = 500
    args.lr = 6e-4 * len(args.gpu.split(','))
    args.bs = 8 * len(args.gpu.split(','))
    args.data = 'qm9'

    x_train, pos_train, y_train = torch.load(f'data_qm9/qm9/train.pt')
    x_val, pos_val, y_val = torch.load(f'data_qm9/qm9/test.pt')
    x_test, pos_test, y_test = torch.load(f'data_qm9/qm9/val.pt')
    ...
    lr_scheduler = opt.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.6, patience=20, min_lr=5e-6)
