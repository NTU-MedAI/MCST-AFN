...
def main():
    ...
    x_train, pos_train, y_train = torch.load(f'data_freesolv/freesolv/train.pt')
    x_val, pos_val, y_val = torch.load(f'data_freesolv/freesolv/test.pt')
    x_test, pos_test, y_test = torch.load(f'data_freesolv/freesolv/val.pt')
    ...
    for epoch in range(0, args.epochs):
        ...
        if  args.data == 'freesolv' and metric < best_metric:
            best_metric = metric
    ...
    torch.save(checkpoint, args.save_path + f'freesolv_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as freesolv_{args.pretrain}_{args.linear_probe}.pt.\n')
