...
def main(number):
    ...
    x_full_dataset, pos_full_dataset, y_full_dataset_1, y_full_dataset_2 = torch.load(f'data_Clintox/Clintox/full.pt')
    ...
    x_train, pos_train, y1_train, y2_train = x_full_dataset[train_indices], pos_full_dataset[train_indices], y_full_dataset_1[
        train_indices], y_full_dataset_2[train_indices]
    y_train = torch.stack((y1_train, y2_train), dim=1)

    x_val, pos_val, y1_val, y2_val = x_full_dataset[val_indices], pos_full_dataset[val_indices], y_full_dataset_1[val_indices], y_full_dataset_2[val_indices]
    y_val = torch.stack((y1_val, y2_val), dim=1)

    x_test, pos_test, y1_test, y2_test = x_full_dataset[test_indices], pos_full_dataset[test_indices], y_full_dataset_1[
        test_indices], y_full_dataset_2[test_indices]
    ...
    torch.save(checkpoint, args.save_path + f'Clintox_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as Clintox_{args.pretrain}_{args.linear_probe}.pt.\n')
