...
from egnn import EGNN_Network, predictor, TEGN
...
def run_eval(args, model, loader, y_gt):
    ...
    with torch.no_grad():
        for x, pos, y in loader:
            ...
            mask = (x != 0)
            # out = model(x, pos, mask=mask)[1][..., 0]  #model[..., 0]选择了model中所有维度上的第一个元素
...
def main():
    ...
    args.epochs = 200
    args.lr = 5e-4 * len(args.gpu.split(',')) #每个学习率不一样
    args.bs = 8 * len(args.gpu.split(','))
    args.data = 'bbbp'

    x_train, pos_train, y_train = torch.load(f'data_bbbp/bbbp/train.pt')
    x_val, pos_val, y_val = torch.load(f'data_bbbp/bbbp/test.pt')
    x_test, pos_test, y_test = torch.load(f'data_bbbp/bbbp/val.pt')
    ...
    try:
        for epoch in range(0, args.epochs):
            ...
            log.logger.info('Epoch: {} | Time: {:.1f}s | Loss: {:.2f} | val_AUROC: {:.3f} | val_AUPRC: {:.3f} | test_AUROC: {:.3f} | test_AUPRC: {:.3f}'
                            '| Lr: {:.3f}'.format(epoch + 1, time() - t1, loss*1e4, auroc, auprc, auroc_test, auprc_test, optimizer.param_groups[0]['lr'] * 1e5))
            lr_scheduler.step(metric)

            if  args.data == 'bbbp' and metric > best_metric:
                best_metric = metric
    ...
    torch.save(checkpoint, args.save_path + f'bbbp_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as bbbp_{args.pretrain}_{args.linear_probe}.pt.\n')
