...
def main(task_num):
    args = parse_args()
    set_seed(args.seed)
    log = Logger(f'{args.save_path}tox21/', f'tox21_{strftime("%Y-%m-%d_%H-%M-%S", localtime())}.log')
    args.epochs = 200
    args.lr = 5e-4 * len(args.gpu.split(','))
    args.bs = 8 * len(args.gpu.split(','))

    #随机切分切分full.pt,切分索引即可
    x_full_dataset, pos_full_dataset, y_full_dataset= torch.load(f'data_tox21/tox21/full_{task_num+1}.pt')
    ...
    # x_train, pos_train, y_train = torch.load(f'data_tox21/tox21/train.pt')
    # x_val, pos_val, y_val = torch.load(f'data_tox21/tox21/test.pt')
    # x_test, pos_test, y_test = torch.load(f'data_tox21/tox21/val.pt')
    ...
    for epoch in range(0, args.epochs):
        ...
        if  args.data == 'tox21' and metric > best_metric:
            best_metric = metric
    ...
    torch.save(checkpoint, args.save_path + f'tox21_{args.pretrain}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as tox21_{args.pretrain}.pt.\n')
...
if __name__ == '__main__':
    ...
    for task_num in range(12):
        print(f'task_num:{task_num+1}')
        auc = main(task_num)
        print(auc)
        auc_list.append(auc)
    best_auc = auc_list.mean()
    print(best_auc)
    # task_num = 11
    # print(f'task_num:{task_num + 1}')
    # auc = main(task_num)
