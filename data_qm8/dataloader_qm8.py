...
with open('./qm8.sdf.csv', 'r') as f:
    target = f.read().split('\n')[1:-1]
    target = [[float(x) for x in line.split(',')[1:13]] for line in target]
...
def qm8_loader(dataset, mode='train'):
    x, pos_sum, y_sum = [], [], []
    for data in dataset:
    ...
    y_sum = tensor(y_sum)
    torch.save([x, pos_sum, y_sum], f'./qm8/{mode}.pt')


qm8_loader(train_dataset,mode='train')
qm8_loader(val_dataset,mode='val')
qm8_loader(test_dataset,mode='test')
