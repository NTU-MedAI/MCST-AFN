...
def main():
    ...
    x_train, pos_train, y_train_ = torch.load(f'data_toxcast/toxcast/train.pt')
    x_val, pos_val, y_val_ = torch.load(f'data_toxcast/toxcast/val.pt')
    x_test, pos_test, y_test_ = torch.load(f'data_toxcast/toxcast/test.pt')
    ...
    torch.save(checkpoint, args.save_path + f'toxcast_{args.pretrain}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as toxcast_{args.pretrain}.pt.\n')
