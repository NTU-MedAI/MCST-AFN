...
class predictor(nn.Module):
    def __init__(self, dim, dropout=0.1):
        ...
        self.out = nn.Sequential(nn.Linear(dim, dim*2), nn.Dropout(p=dropout), nn.GELU(), nn.Linear(dim*2, 21))
...
def main(number):
    ...
    x_full_dataset, pos_full_dataset, y_full_dataset= torch.load(f'data_toxcast/toxcast/full.pt')
    ...
    for epoch in range(0, args.epochs):
        ...
        if  args.data == 'toxcast' and metric > best_metric:
            best_metric = metric
    ...
    torch.save(checkpoint, args.save_path + f'toxcast_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as toxcast_{args.pretrain}_{args.linear_probe}.pt.\n')
