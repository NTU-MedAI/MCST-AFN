...
def main():
    ...
    log = Logger(f'{args.save_path}qm7/', f'qm7_{strftime("%Y-%m-%d_%H-%M-%S", localtime())}.log')
    args.epochs = 500
    args.lr = 5e-4 * len(args.gpu.split(','))
    args.bs = 16 * len(args.gpu.split(','))
    args.data = 'qm7'

    x_train, pos_train, y_train = torch.load(f'data_qm7/qm7/train.pt')
    x_val, pos_val, y_val = torch.load(f'data_qm7/qm7/val.pt')
    x_test, pos_test, y_test = torch.load(f'data_qm7/qm7/test.pt')
    ...
    torch.save(checkpoint, args.save_path + f'qm7_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as qm7_{args.pretrain}_{args.linear_probe}.pt.\n')
    log.logger.info('Best Epoch: {} | Test MAE: {:.3f}'.format(best_epoch, best_test_mae))
