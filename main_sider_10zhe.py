...
class predictor(nn.Module):
    def __init__(self, dim, dropout=0.1):
        ...
        self.out = nn.Sequential(nn.Linear(dim, dim*2), nn.Dropout(p=dropout), nn.GELU(), nn.Linear(dim*2, 27))
...
def main(number):
    ...
    args.epochs = 200
    args.lr = 1e-3 * len(args.gpu.split(','))
    args.bs = 8 * len(args.gpu.split(','))
    args.data = 'sider'

    # 随机切分切分full.pt,切分索引即可
    x_full_dataset, pos_full_dataset, y_full_dataset= torch.load(f'data_sider/sider/full.pt')
    ...
    x_train, pos_train, y_train_= x_full_dataset[train_indices], pos_full_dataset[train_indices], y_full_dataset[
        :,train_indices]

    x_val, pos_val, y_val_ = x_full_dataset[val_indices], pos_full_dataset[val_indices], y_full_dataset[:,val_indices]

    x_test, pos_test, y_test_ = x_full_dataset[test_indices], pos_full_dataset[test_indices], y_full_dataset[:,
        test_indices]
    ...
    for epoch in range(0, args.epochs):
        ...
        if  args.data == 'sider' and metric > best_metric:
            best_metric = metric
    ...
    torch.save(checkpoint, args.save_path + f'sider_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as sider_{args.pretrain}_{args.linear_probe}.pt.\n')
