...
def main():
    ...
    x_train, pos_train, y_train_ = torch.load(f'data_sider/sider/train.pt')
    x_val, pos_val, y_val_ = torch.load(f'data_sider/sider/test.pt')
    x_test, pos_test, y_test_ = torch.load(f'data_sider/sider/val.pt')
    ...
    torch.save(checkpoint, args.save_path + f'sider_{args.pretrain}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as sider_{args.pretrain}.pt.\n')
