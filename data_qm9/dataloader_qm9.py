...
with open('./gdb9.sdf.csv', 'r') as f:
    target = f.read().split('\n')[1:-1]
    target = [[float(x) for x in line.split(',')[6:9]] for line in target]
...
def qm9_loader(dataset, mode='train'):
    x, pos_sum, y_sum = [], [], []
    for data in dataset:
    ...
    y_sum = tensor(y_sum)
    torch.save([x, pos_sum, y_sum], f'./qm9/{mode}.pt')


qm9_loader(train_dataset,mode='train')
qm9_loader(val_dataset,mode='val')
qm9_loader(test_dataset,mode='test')
