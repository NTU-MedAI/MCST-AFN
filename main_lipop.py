...
def main():
    ...
    log = Logger(f'{args.save_path}lipop/', f'lipop_{strftime("%Y-%m-%d_%H-%M-%S", localtime())}.log')
    args.epochs = 200
    args.lr = 5e-4 * len(args.gpu.split(','))
    args.bs = 8 * len(args.gpu.split(','))
    args.data = 'lipop'

    x_train, pos_train, y_train = torch.load(f'data_lipop/lipop/train.pt')
    x_val, pos_val, y_val = torch.load(f'data_lipop/lipop/test.pt')
    x_test, pos_test, y_test = torch.load(f'data_lipop/lipop/val.pt')
    ...
    for epoch in range(0, args.epochs):
        ...
        spearman_test, pearson_test, metric_test = run_eval(args, model, test_loader, y_test)
        mse_list.append(metric_test)

        log.logger.info(
            'Epoch: {} | Time: {:.1f}s | Loss: {:.2f} | RMSE: {:.3f} | Pearson: {:.3f} | Spearman: {:.3f} | test_RMSE: {:.3f} | test_Pearson: {:.3f} | test_Spearman: {:.3f} '
                        '| Lr: {:.3f}'.format(epoch + 1, time() - t1, loss * 1e4, metric , pearson, spearman, metric_test, pearson_test, spearman_test,
                                              optimizer.param_groups[0]['lr'] * 1e5))

        lr_scheduler.step(metric)

        if  args.data == 'lipop' and metric < best_metric:
            best_metric = metric
    ...
    best_test_mse = min(mse_list)
    best_epoch = mse_list.index(best_test_mse) + 1
    ...
    torch.save(checkpoint, args.save_path + f'lipop_{args.pretrain}_{args.linear_probe}.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as lipop_{args.pretrain}_{args.linear_probe}.pt.\n')
    log.logger.info('Best Epoch: {} | Test MSE: {:.3f}'.format(best_epoch, best_test_mse))
